...
public class Targeter : MonoBehaviour
{
    ...
    public Enemy GetClosestEnemy(Vector3 point)//Finds the enemy closest to a given point 
    //Handles null references and removes dead enemies from the List.
    {
    ...
    //Unity events:
    void OnTriggerEnter(Collider other)//Adds enemy to the List when they enter the collider.
    {
        var enemy = other.gameObject.GetComponent<Enemy>();
        if (enemy != null)
            enemies.Add(enemy);
    }
    void OnTriggerExit(Collider other)//Adds enemy to the List when they enter the collider.
    {
        var enemy = other.gameObject.GetComponent<Enemy>();
        if (enemy != null)
