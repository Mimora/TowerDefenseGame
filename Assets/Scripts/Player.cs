...
{
    private enum Mode //distinguish between build mode and play mode
    {
        Build,
        Play
    }
    ...
    [Tooltip("Reference to the lives display text.")]
    ...
    void UpdateEnemyPath() {
        Invoke("PerformPathfinding",.1f); //Invoke Pathfinding with a slight delay
    }
    ...
    void Start()
    {
        ...
        trans.position = targetPosition; // ✅ 强制应用初始视角
    ...
    void UpdateRemainingLives()
    {
        remainingLivesText.text = remainingLives + " lives";
    }
    ...
    void Update()
    {
        ...
        // {
        //     gameLostPanel.SetActive(true);
        //     gameLostPanelInfoText.text = "手动触发 Game Over 面板";
        //     Debug.Log("✅ Game Over UI 手动触发！");
        // }
        ArrowKeyMovement();
